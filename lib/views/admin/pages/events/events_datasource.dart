    ...
    return DataRow.byIndex(index: index, cells: [
      ...
      DataCell(Row(
        children: [
          ...
          if (session.permisions.where((e) => e.name == 'Ver reportes de eventos').isNotEmpty)
            IconButton(icon: const Icon(Icons.remove_red_eye), onPressed: () => showEvent(event)),
          if (session.permisions.where((e) => e.name == 'Editar eventos').isNotEmpty)
            IconButton(icon: const Icon(Icons.edit_outlined), onPressed: () => editTypeUser(event)),
          if (session.permisions.where((e) => e.name == 'Eliminar eventos').isNotEmpty)
            Transform.scale(
                scale: .5,
                child: Switch(
